      ...
      <MajiDialogue
        turns={[
          ...
          },
          {
            speaker: "master",
            emotion: "explain",
            text: "良い問いです、マジさん。これは航空機のパイロット訓練に近い話です。\n彼らは「エンジンが止まったら？」「片翼に氷がついたら？」を事前に何度も訓練します。\nエラーハンドリングとはコードの世界でこれを行うこと——失敗を「想定外」ではなく「想定内の出来事」として扱う設計です。\n`try` で通常飛行、`catch` で緊急時マニュアル、`finally` で着陸後のチェックリスト、というわけです。",
          },
          {
            speaker: "maji",
            emotion: "question",
          ...
          },
          {
            speaker: "master",
            emotion: "standard",
            text: "はい、その通りです。例えばファイルを開いたら必ず閉じる、データベースに接続したら必ず切る、ローディング画面を出したら必ず消す——\nこうした「成功しても失敗しても、必ず後始末しなければならない処理」を入れる場所が `finally` です。\nレストランで料理が出ても出なくても会計はする、というのと同じですね。",
          },
          {
            speaker: "maji",
            emotion: "surprised",
          ...
          },
          {
            speaker: "master",
            emotion: "explain",
            text: "そこに気づけたのが大きな前進です、マジさん。エラーハンドリングはユーザー体験を守る最後の砦です。\n「起きてから考える」のではなく、「設計時に、起きた時にどう振る舞うかまで決めておく」。\n設計の段階でエラーを想定しておく習慣がつくと、あなたのコードは格段に信頼されるものになります。",
          },
        ]}
      />
