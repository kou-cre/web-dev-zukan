      ...
      <MajiDialogue
        turns={[
          ...
          },
          {
            speaker: "master",
            emotion: "explain",
            text: "良いご質問ですね、マジさん。これはロッカールームに3種類の棚があると思ってください。\n`var` は廊下にポンと置いた荷物、`let` は個人ロッカー、`const` は鍵のかかったロッカーです。\n歴史的には `var` しかなかった時代があり、廊下に荷物を置きすぎて誰の物か分からなくなった反省から、`let` と `const` が後から追加されたんです。",
          },
          {
            speaker: "maji",
            emotion: "question",
          ...
          },
          {
            speaker: "master",
            emotion: "standard",
            text: "落ち着いてください、マジさん。半分正解で、半分は誤解です。\n`const` が固定しているのは「箱の置き場所」であって、「箱の中身」ではないのです。\nたとえば `const user = { name: 'マジ' }` と書いた場合、user という変数に別のオブジェクトを再代入することはできません。\nしかし user.name を 'マスター' に書き換えるのは自由にできます。\n封筒の宛先は変えられないが、封筒の中身は入れ替えられる、というイメージですね。",
          },
          {
            speaker: "maji",
            emotion: "worried",
            text: "あ、あぁ……なんとか分かった気がします。\nじゃあ次に「スコープ」っていうやつなんですけど……ボクここがどうしても腑に落ちなくて。\nなぜ外から中が見えないんですか？ 透明にしておいてくれた方が便利なのでは……？",
          },
          {
            speaker: "master",
            emotion: "thinking",
            text: "それも素晴らしい疑問です。会社のフロア構造を想像してみてください。\nマジさんが3階の経営フロアにいるとして、1階の受付の資料は取りに行けますよね。\nでも、1階の警備員が3階の社長室に勝手に入ってきたら困りませんか？\nスコープも同じで、内側から外を見るのは安全だが、外から内に踏み込まれると秩序が崩れる。\nだからJavaScriptは「外から中は見えない」ようにできているんです。\nこれがあるおかげで、関数の中で使う一時的な変数が、外の世界を汚さずに済むんですよ。",
          },
          {
            speaker: "maji",
            emotion: "standard",
            text: "なるほど……。「外から中が見えない」のは不便なのではなくて、「中で使った道具が外を汚さない」ための仕組みだったんですね。\n`var` は廊下に放置だから事故が起きる、`let` と `const` はちゃんと部屋の中に閉じ込められる。\nやっと繋がってきました。",
          },
          {
            speaker: "master",
            emotion: "explain",
            text: "見事にまとめていただきました、マジさん。最後に実践ルールをひとつ。\n「迷ったらまず `const` で書き、後から値を変える必要が出てきたら `let` に直す」。\nこれだけ守れば、変数まわりで足を取られることはまずありません。\n`var` のことはレガシーコードを読むときの予備知識として頭の片隅に置いておけば十分ですよ。",
          },
        ]}
      />
